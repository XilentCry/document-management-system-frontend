import { TFilterType } from "@/features/items/types/filter-type";
import {
  keepPreviousData,
  useInfiniteQuery,
  useQuery,
...
import { TFilterOwner } from "@/features/items/types/filter-owner";
...
export const useSearchSpecificUsers = (
  ...
  const {
    ...
    queryKey: ["organization-units", "detail", id, "specific-users", { search: searchTerm }],
...
export const useGetAllOrganizationUnitsFlat = (page: number, searchTerm?: string) => {
  const { isLoading, isError, error, isSuccess, data, isPlaceholderData } =
    useQuery({
      queryKey: ["organization-units", { page, search: searchTerm }],
...
export const useGetOrganizationUnitItems = (id: string) => {
  const {
    ...
    queryKey: ["organization-units", "detail", id, "items"],
...
export const useGetOrganizationUnitFolders = (
  ...
  const {
    ...
    queryKey: ["organization-units", "detail", organizationUnitId, "folders"],
    queryFn: ({ pageParam }) =>
